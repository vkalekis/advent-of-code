...
func (s *Solver2024) Day_08(part int, reader utils.Reader) int {
	...
	for c, as := range antennas {
		...
		utils.Logger.Infof("Label:%s Antennas:%v Pairs:%v", c, as, pairs)
		...
		case 1:
			for _, pair := range pairs {
				...
				utils.Logger.Infof("Pair: %v Antinode: %v", pair, antinode)
		...
		case 2:
			for _, pair := range pairs {
				...
				if pair[1].Row >= 0 && pair[1].Row < rows && pair[1].Col >= 0 && pair[1].Col < cols {
					utils.Logger.Infof("Pair: %v Antinode (pair1): %v", pair, pair[1])
