...
func (cur *node10) lookupneighbors(lines map[int][]string, visitedCoords map[string]*node10, row, col, maxRows, maxCols int) {
	...
	logger.Infof(">>> Current: (%v %v) %v %+v", row, col, cur.label, cur)
	...
	for _, n := range cur.getNeightbors() {
		...
		logger.Infof(">>>> Debug coordinates of neighbords of (%v,%v): (%v,%v) in %v ", row, col, nrow, ncol, dir)
		if neigh, ok := visitedCoords[ncoords]; !ok {
			...
			found := false
			logger.Infof("???? %v %v", n.labels, lines[nrow][ncol])
			for _, v := range n.labels {
				logger.Infof("???? %v %v", v, lines[nrow][ncol])
			...
			logger.Infof(">>>>>> Here: %v (%v,%v) (%v,%v) %v -> %v", dir, row, col, nrow, ncol, cur.label, neigh.label)
			logger.Infof(">>>>>> Here: %v (%v,%v) (%v,%v) %p %p", dir, row, col, nrow, ncol, cur, neigh)
			...
			logger.Infof(">>>>>> Here1: %v (%v %v) (%v %v) %v -> %v", dir, row, col, nrow, ncol, cur.label, neigh.label)
...
func buildGraph(lines map[int][]string, maxRows, maxCols int) (map[string]*node10, map[string]interface{}, utils.Coordinates) {
	...
	for row := 0; row < maxRows-1; row++ {
		...
		for col, c := range line {
			...
			logger.Infof("(%v,%v) %v", row, col, cur.label)
...
func (root *node10) do_bfs() int {
	...
	logger.Infof("Queue: %+v %v", q.Items(), q.IsEmpty())

	for !q.IsEmpty() {
		n, ok := q.Dequeue()
		logger.Debugf("Currently on: (%v,%v) %p %+v", n.coords.Row, n.coords.Col, n, n)
...
func (s Solver2023) Day_10(part int, reader utils.Reader) int {
	...
	logger.Debugf("Lines: %+v MR: %v MC: %v", lines, maxRows, maxCols)
	...
	logger.Infof("S is here: (%v,%v) and is: %v",
	...
	if err := utils.WriteMapToFile(output, "outputs/output10.json"); err != nil {
		logger.Errorf("Could not write output to file %s: %v", "output10.json", err)
	}
	...
	case 2:
		...
		for gr, _ := range grounds {
			...
			row := utils.ToInt(strings.Split(gr, "-")[0])
			col := utils.ToInt(strings.Split(gr, "-")[1])

			countL, countR := loc(visitedCoords, row, col, maxRows, maxCols)

			if countL%2 == 1 && countR%2 == 1 {
				logger.Infof("%v %v    %v %v", row, col, countR, countR)
				total++
			}
		}
		...
		for _, v := range visitedCoords {
			if !v.visited {
				...
				if countL%2 == 1 && countR%2 == 1 {
					logger.Infof("%v %v    %v %v", v.coords.Row, v.coords, countL, countR)
					total++
				}
			}
		}
