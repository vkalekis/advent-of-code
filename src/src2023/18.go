...
func traverseLagoon(lines []line16) int {
	...
	for _, line := range lines {
		...
		logger.Infof("Shifting: %+v %v", line, coords[ind])
	...
	for ind := 1; ind < len(coords); ind++ {
		...
		logger.Infof("%+v->%+v %d", coords[ind-1], coords[ind], factor)
		area += factor
	...
	logger.Infof("%+v", 0.5*float64(area))
	logger.Infof("%+v", boundaries)
...
func (s Solver) Day_18(part int, reader utils.Reader) int {
	...
	case 1:
		...
		for line := range reader.Stream() {
			...
			steps := utils.ToInt(splittedLine[1])
	...
	case 2:
		...
		for line := range reader.Stream() {
			...
			logger.Debugf("%s %d", hex[1:6], steps)
