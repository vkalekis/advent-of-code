...
func (s Solver2023) Day_16(part int, reader utils.Reader) int {
	...
	case 1:
		...
		for !q.IsEmpty() {
			...
			logger.Infof("Poped: %+v %s - Q: %v", n, grid[n.coords.Row][n.coords.Col], q.Items())
			...
			logger.Infof("Defl: %v", q.Items())
			logger.Infof("GridR %d GridC: %d", n.coords.Row, n.coords.Col)

			// logger.Infoln(" ", cols)
			// for r := 0; r < maxRows; r++ {
			// 	logger.Infoln(rows[r], grid[r])
			// }
	...
	case 2:
		...
		for _, start := range startNodes {
			...
			logger.Infof("Starting with %+v", start)
			...
			go func(localGrid map[int][]string, start node16) {
				...
				for !q.IsEmpty() {
					...
					logger.Debugf("Poped: %+v %s - Q: %v", n, localGrid[n.coords.Row][n.coords.Col], q.Items())
					...
					logger.Debugf("Defl: %v", q.Items())
					logger.Debugf("GridR %d GridC: %d", n.coords.Row, n.coords.Col)
				}
