...
func parseLines20(reader utils.Reader) (broadcaster, map[string]component) {
	...
	for line := range reader.Stream() {
		...
		utils.Logger.Debugf("Line: %v", line)
		...
		if len(match) > 2 {
			...
			utils.Logger.Debugf("L: %v R: %v", left, right)
	...
	for _, cj := range cjs {
		for _, c := range components {
			...
			case *conjuction:
			default:
				utils.Logger.Errorf("Unknown type")
				panic("unknown type")
			}
		}
...
func pushButton(components map[string]component, b broadcaster) (int, int) {
	...
	for !q.IsEmpty() {
		...
		utils.Logger.Debugf("Popped: %+v", pulse)
...
func (s Solver2023) Day_20(part int, reader utils.Reader) int {
	...
	utils.Logger.Infof("Broadcaster: %+v", b)
	for _, c := range components {
		utils.Logger.Infof("Component: %+v", c)
	}
	...
	case 1:
		...
		for buttonPress < maxButtonPresses {
			...
			for _, c := range components {
				utils.Logger.Debugf("Component: %+v", c)
			}
			buttonPress++
		...
		utils.Logger.Infof("High pulses: %d Low Pulses: %d", highPulses, lowPulses)
	...
	case 2:
		...
		// 	if output, ok := components["rx"].(*output); ok {
		// 		utils.Logger.Infof("Rx: %d", output.state)
